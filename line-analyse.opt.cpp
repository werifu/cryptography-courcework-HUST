...
int main() {
    ...
    for (int i_ = 0; i_ < n; i_++) {
        ...
        for (int i = 0; i < 8000; i++) {
            ...
            for (int l1 = 0; l1 < 16; l1++) {
                for (int l2 = 0; l2 < 16; l2++) {
                    uint u4_2 = rsbox[l1 ^ ((y >> 8) & 0xf)];
                    uint u4_4 = rsbox[l2 ^ (y & 0xf)];
                    // one chain from the textbook:
                    // X_5 ^ X_7 ^ X_8 ^ U4_6 ^ U4_8 ^ U4_14 ^ U4_16
                    // its ksai is ±1/32
                    count[l1][l2] +=
                        !(((x >> 11) & 1) ^ ((x >> 9) & 1) ^ ((x >> 8) & 1) ^
                          ((u4_2 >> 2) & 1) ^ ((u4_2)&1) ^ ((u4_4 >> 2) & 1) ^
                          ((u4_4)&1));
                }
            }
        }
        ...
        // The second chain
        // X_5 ^ X_6 ^ X_7 ^ X_8 ^ U4_2 ^ U4_4 ^ U4_6 ^ U4_8 ^ U4_ 10 ^ U4_12
        for (int blk2 = 0; blk2 < 256; blk2++) {
            ...
            for (int i = 0; i < 8000; i++) {
                ...
                for (int l1 = 0; l1 < 16; l1++) {
                    for (int l2 = 0; l2 < 16; l2++) {
                        uint u4_1 = rsbox[l1 ^ ((y >> 12) & 0xf)];
                        uint u4_2 = rsbox[block2 ^ ((y >> 8) & 0xf)];
                        uint u4_3 = rsbox[l2 ^ ((y >> 4) & 0xf)];
                        count2[l1][l2] += !(((x >> 11) & 1) ^ ((x >> 10) & 1) ^
                                            ((x >> 9) & 1) ^ ((x >> 8) & 1) ^
                                            ((u4_1 >> 2) & 1) ^ (u4_1 & 1) ^
                                            ((u4_2 >> 2) & 1) ^ (u4_2 & 1) ^
                                            ((u4_3 >> 2) & 1) ^ (u4_3 & 1));
