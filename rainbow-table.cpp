...
int UnitSHA1(const char* str, int length, uint sha1[5]) {
    /**
     * 计算字符串SHA-1
     * 参数说明：
     * str         字符串指针
    ...
    if (!(pp = (unsigned char*)malloc((uint)l))) return -1;
...
// reduction function
void R(uint sha1[5], char str[8], int i) {
...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (hmap.find(tail_str) != hmap.end()) {
            printf("duplicated! %8s\n", tail[i]);
        }
